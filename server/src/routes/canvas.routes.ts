...
export function mountRoutes(router: Router): void {
  ...
  // Place pixel (no payment yet in Phase 1 - will be added in Phase 2)
  router.post("/api/canvas/pixel", (req, res) => {
    try {
      ...
      const price = getPixelPrice(x, y);
      const pixel = canvasService.setPixel(x, y, color, "anonymous", price);
      if (!pixel) return res.status(400).json({ error: "OUT_OF_BOUNDS" });
